...
<div id="element" class="relative w-5/6 h-screen overflow-y-auto">
  <div class="flex flex-col pb-5">
      ...
      <div class="flex flex-col justify-center items-center">
        ...
        <div class="flex flex-row justify-between items-center w-5/6">
          <div class="flex flex-row ">
            <img class="border-4 border-gray-200 rounded-full w-48 h-48 -mt-10 shadow-xl object-cover"  style="border-color: {{Auth::user()->messenger_color}}"
            ...
            <div class="flex flex-col justify-center items-center pl-5">
              <h1 style="color: {{Auth::user()->messenger_color}}" class="font-bold text-3xl text-center my-3 capitalize">{{ Auth::user()->name }} {{ Auth::user()->surname }}</h1>              
            </div>
          </div>
          <div class="p-4 flex flex-row text-center h-full">
            <ul class="flex flex-row justify-around">
              <li class="flex flex-col px-2">
                <a class="hover:text-white font-semibold block text-gray-200 text-2xl mb-1">{{$posts->count()}}</a>
                <h3 class="text-gray-300 text-lg">Posts</h3>
              </li>
              <li class="flex flex-col px-2">
                <a class="hover:text-white font-semibold mb-1 block text-gray-200 text-2xl">{{Auth::user()->friends()->count()}}</a>
                <h3 class="text-gray-300 text-lg">Friends</h3>
              </li>
              <li class="flex flex-col px-2">
                <a class="hover:text-white font-semibold mb-1 block text-gray-200 text-2xl">{{Auth::user()->friendRequests()->count()}}</a>
    ...
    <div class="container flex flex-col w-full h-full">
      <form class="container p-5 bg-slate-200 rounded-3xl border-4" method="POST" action="/update" enctype="multipart/form-data"  style="border-color: {{Auth::user()->messenger_color}}">
        @csrf
